...
namespace Contextual
{
    public partial class frmPrograms : Form
    {
        ...
        private void dataPrg_DoubleClick(object sender, EventArgs e)
        {
            ...
            //programPage.txtPrgName.Text = this.dataPrg.CurrentRow.Cells[1].Value.ToString();

            //programPage.txtPrgAbbr.Text = this.dataPrg.CurrentRow.Cells[2].Value.ToString();

            programPage.Show();


            /*
            frmDashboard dash = new();

            /dash.pnlLoader.Controls.Clear();
            
            frmProgramData programPage = new frmProgramData() { Dock = DockStyle.Fill, TopLevel = false, TopMost = true };
            programPage.label17.Text = this.dataPrg.CurrentRow.Cells[1].Value.ToString();
            programPage.FormBorderStyle = FormBorderStyle.None;
            dash.pnlLoader.Controls.Add(programPage);
            programPage.Show();
            */
